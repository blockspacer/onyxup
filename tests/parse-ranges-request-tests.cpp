...
public:
    static std::vector<std::pair<size_t , size_t >> parseRangesRequestTest(const std::string & src, size_t max_index_byte){
        return onyxup::HttpServer::parseRangesRequest(src, max_index_byte);
    }
};
...
TEST_F(ParseRangesRequestTests, Test_1) {
    ...
    std::vector<std::pair<size_t , size_t >> ranges = WrapperHttpServer::parseRangesRequestTest(src, 20000);
...
TEST_F(ParseRangesRequestTests, Test_25) {
    ...
    std::vector<std::pair<size_t , size_t >> ranges = WrapperHttpServer::parseRangesRequestTest(src, 1);
    ASSERT_EQ(ranges[0].first, 0);
...
TEST_F(ParseRangesRequestTests, Test_26) {
    std::string src = "bytes=0-0";
    try{
        std::vector<std::pair<size_t , size_t >> ranges = WrapperHttpServer::parseRangesRequestTest(src, 0);
        ASSERT_EQ(true, false);
    } catch (onyxup::OnyxupException & ex){
        ASSERT_EQ(true, true);
    }
